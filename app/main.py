...
from app.routers import auth_db as auth, analysis, invite, invite_code, payment, screening, queue, sse, health, favorites, config, reports, database, operation_logs, stock_pitch, tags, tushare_init, akshare_init, baostock_init, historical_data, multi_period_sync, financial_data, news_data, social_media, internal_messages, usage_statistics, model_capabilities, cache, logs, wechat_qrcode_invite
...
async def _print_config_summary(logger):
    ...
    try:
        logger.info("=" * 70)
        logger.info("📋 FinAgents Configuration Summary")
        logger.info("=" * 70)
...
app.include_router(invite_code.router, prefix="/api", tags=["传统邀请码"])
app.include_router(invite.router, prefix="/api", tags=["传统邀请码"])
