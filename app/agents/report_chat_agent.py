...
def calc_token_cost(total_tokens: int) -> Decimal:
    ...
    cost = Decimal(str(total_tokens)) * TOKEN_COST_RATE * Decimal("2.5")
...
def build_default_tools() -> ToolRegistry:
    ...
    registry.register(
        "fetch_historical",
        "获取指定股票的历史行情数据。参数: symbol(股票代码), start_date(开始日期YYYY-MM-DD,必传), end_date(结束日期YYYY-MM-DD,必传), period(周期,daily/weekly/monthly,默认daily), limit(条数,默认30)",
    ...
    registry.register(
        ...
        "EMA穿透买入策略分析工具，基于均线穿透方法计算建议买入价。参数: symbol(6位股票代码), fast_ema(快EMA周期,默认13), slow_ema(慢EMA周期,默认26), lookback_period(穿透回溯天数,默认30), start_date(开始日期YYYY-MM-DD,必传), end_date(结束日期YYYY-MM-DD,必传)",
