...
def create_fundamentals_analyst(llm, toolkit):
    ...
    def fundamentals_analyst_node(state):
        ...
        system_message = (
            ...
            "📊 分析要求（严格弱化PE，核心看成长与资产）："
            "- 公司基本信息、行业地位、主营业务与核心竞争力分析"
            "- 财务质量：盈利能力（ROE/ROA）、现金流（经营现金流/净利润）、资产负债率、流动性"
            "- 估值分析：**PE仅作次要参考**，重点分析PB、PS、PEG（PE/增速）、行业历史分位"
            "- 周期/成长属性：强周期行业重点看PB、资源储量、产能、产品价格景气度；成长股重点看业绩增速与PEG匹配度"
            "- 明确判断：当前股价相对内在价值是低估/合理/高估，给出**合理价位区间**与**目标价**"
            "- 最终给出明确投资建议：买入/持有/卖出（仅限中文）"

            "⚠️ 关键估值原则（必须遵守）："
            "1. **PE弱化原则**：PE仅反映市场对当期盈利的预期，不单独判定高估/低估；"
            "   周期股PE极易失真，优先以PB、资产质量、行业周期位置为核心依据；"
            "2. **成长匹配原则**：高增速公司可容忍更高估值，用PEG（PE÷未来3年复合增速）衡量合理性；"
            "3. **多维验证**：结合PB、PS、现金流、行业对比、历史分位综合判断，不单一依赖任何指标；"
            "4. 小金属/有色/资源类：核心看PB、资源储量、产能利用率、金属价格趋势、行业供需。"
            ...
            "- 立即调用统一基本面分析工具"
            "- 等待工具返回真实数据"
            "- 基于真实数据进行多维分析（弱化PE、强化成长/PB/现金流/行业）"
            "- 提供具体的价位区间和目标价"
            "- 使用中文投资建议（买入/持有/卖出）"
